...
public class DeliveryRepository : GenericRepository<Delivery>, IDeliveryRepository
{
    ...
    public async Task<Delivery?> GetByIdWithGroupOrderAsync(Guid id, CancellationToken cancellationToken = default)
    {
        return await DbSet
            ...
            .Include(d => d.GroupOrder).ThenInclude(go => go.Invoices).ThenInclude(i => i.Buyer).ThenInclude(b => b.User)
            .Include(d => d.GroupOrder).ThenInclude(go => go.Invoices).ThenInclude(i => i.Buyer).ThenInclude(b => b.Region)
